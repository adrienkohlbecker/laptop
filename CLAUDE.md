...
- **dotfiles** — a separate repo, deployed with GNU stow. `install.sh` clones it to `~/Desktop/dotfiles`, and that is the directory the `dotfiles` step stows from.
...
1. `bootstrap` — FileVault (instant on Apple Silicon, no reboot; `fdesetup -inputplist` in the VM, interactive prompt otherwise), Command Line Tools (headless via the `softwareupdate` install-on-demand trick), Homebrew (installs + `brew shellenv` onto PATH), and cloning the dotfiles repo to `~/Desktop/dotfiles`.
...
3. `dotfiles` — `stow --restow */` from `~/Desktop/dotfiles` into `$HOME`. **Must follow `packages`** (needs the `stow` binary) and **precede `runtimes`** (deploys `~/.config/mise/config.toml` + `config.mac.toml` and the `~/.default-*` lists mise reads).
...
- **macOS `defaults` aren't reloaded** by the `settings` step (no `killall Dock`/`SystemUIServer`); they take effect on next login, matching the previous behaviour.
